...
2. **環境變數設定**
   ...
   cp .env.example .env
   ```
   確保 `.env` 中的 `EXPO_PUBLIC_API_URL` 正確指向後端 API（例如 `http://127.0.0.1:8000/api`）。若是用實體手機測試，需更改為您的電腦區域網路 IP。
3. **啟動 Expo 開發伺服器**
   ```bash
   npx expo start -c
   ```
   您可以掃描終端機中的 QR Code 透過 **Expo Go** 應用程式在手機上預覽，或是按 `w` 鍵開啟網頁版本 (`http://localhost:8081`)。
